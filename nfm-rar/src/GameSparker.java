...
public class GameSparker extends Applet implements Runnable {
   ...
   public void loadbase(ContO[] aconto, Medium medium, Trackers trackers) {
      ...
      try {
         URL url = new URL(this.getRoot(), "graphics/models.zipo");
         DataInputStream datainputstream = new DataInputStream(url.openStream());
         ZipInputStream zipinputstream = new ZipInputStream(datainputstream);
         ZipEntry zipentry = zipinputstream.getNextEntry();

         for (Object obj = null; zipentry != null; zipentry = zipinputstream.getNextEntry()) {
            int i = 0;
            int j = 0;
            ...
            while (j > 0) {
               int l = zipinputstream.read(abyte0, k, j);
               k += l;
               j -= l;
            }
         ...
         System.out.println("Error Reading Models: " + var15);
      }
   ...
   public void loadstage(
      ContO[] aconto, ContO[] aconto1, Medium medium, Trackers trackers, CheckPoints checkpoints, xtGraphics xtgraphics, Madness[] amadness, Record record
   ) {
      trackers.nt = 0;
      this.nob = 5;
      this.notb = 0;
      ...
      try {
         URL url = new URL(this.getRoot(), "stages/" + checkpoints.stage + ".txt");
         DataInputStream datainputstream = new DataInputStream(url.openStream());
         ...
         while ((s = datainputstream.readLine()) != null) {
            ...
            if (s1.startsWith("set")) {
               ...
               aconto[this.nob] = new ContO(
                  aconto1[k], this.getint("set", s1, 1), this.getint("set", s1, 3), this.getint("set", s1, 2), this.getint("set", s1, 4)
               );
               if (s1.indexOf(")p") != -1) {
                  checkpoints.x[checkpoints.n] = this.getint("chk", s1, 1);
                  checkpoints.z[checkpoints.n] = this.getint("chk", s1, 2);
                  checkpoints.y[checkpoints.n] = this.getint("chk", s1, 3);
            ...
            if (s1.startsWith("chk")) {
               ...
               aconto[this.nob] = new ContO(
                  aconto1[l], this.getint("chk", s1, 1), this.getint("chk", s1, 3), this.getint("chk", s1, 2), this.getint("chk", s1, 4)
               );
               checkpoints.x[checkpoints.n] = this.getint("chk", s1, 1);
               checkpoints.z[checkpoints.n] = this.getint("chk", s1, 2);
               checkpoints.y[checkpoints.n] = this.getint("chk", s1, 3);
               if (this.getint("chk", s1, 4) == 0) {
                  checkpoints.typ[checkpoints.n] = 1;
               } else {
                  checkpoints.typ[checkpoints.n] = 2;
               }
            ...
            if (s1.startsWith("fix")) {
               ...
               aconto[this.nob] = new ContO(
                  aconto1[i1], this.getint("fix", s1, 1), this.getint("fix", s1, 3), this.getint("fix", s1, 2), this.getint("fix", s1, 4)
               );
            ...
            if (s1.startsWith("maxr")) {
               ...
               for (int j4 = 0; j4 < j1; j4++) {
                  aconto[this.nob] = new ContO(aconto1[59], j2, medium.ground - aconto1[59].grat, j4 * 4800 + j3, 0);
                  this.nob++;
               }
            ...
            if (s1.startsWith("maxl")) {
               ...
               for (int k4 = 0; k4 < k1; k4++) {
                  aconto[this.nob] = new ContO(aconto1[59], k2, medium.ground - aconto1[59].grat, k4 * 4800 + k3, 0);
                  this.nob++;
               }
            ...
            if (s1.startsWith("maxt")) {
               ...
               for (int l4 = 0; l4 < l1; l4++) {
                  aconto[this.nob] = new ContO(aconto1[59], l4 * 4800 + l3, medium.ground - aconto1[59].grat, l2, 90);
                  this.nob++;
               }
            ...
            if (s1.startsWith("maxb")) {
               ...
               for (int i5 = 0; i5 < i2; i5++) {
                  aconto[this.nob] = new ContO(aconto1[59], i5 * 4800 + i4, medium.ground - aconto1[59].grat, i3, 90);
                  this.nob++;
               }
      ...
      if (xtgraphics.fase == 2) {
         ...
         if (trackers.nt >= 4) {
            ...
            do {
               medium.trx = medium.trx + trackers.x[trackers.nt - i];
               medium.trz = medium.trz + trackers.z[trackers.nt - i];
            } while (--i > 0);
         }
   ...
   public void run() {
      ...
      float f = 35.0F;
      int l = 80;
      var35 = this.readcookie("unlocked");
      if (var35 >= 1 && var35 <= 50) {
         xtgraphics.unlocked = 50;
         if (xtgraphics.unlocked != 50) {
            checkpoints.stage = xtgraphics.unlocked;
         } else {
            checkpoints.stage = (int)(Math.random() * 50.0) + 1;
         }

         xtgraphics.opselect = 0;
      }

      var35 = this.readcookie("usercar");
      if (var35 >= 0 && var35 <= 30) {
         xtgraphics.sc[0] = 24;
      }

      var35 = this.readcookie("gameprfact");
      if (var35 != -1) {
      ...
      while (true) {
         ...
         if (xtgraphics.fase == -5) {
            ...
            if (flag) {
               if (checkpoints.stage == xtgraphics.unlocked && xtgraphics.winner && xtgraphics.unlocked != 50) {
         ...
         if (xtgraphics.fase == 0) {
            ...
            if (xtgraphics.starcnt < 35) {
               ...
               if (this.view == 1) {
                  medium.lead(aconto1[0], amadness[0].cxz);
                  xtgraphics.stat(amadness[0], checkpoints, this.u[0], true, this.rd);
               }
               ...
               if (this.view == 3) {
                  ...
                  if (this.view == 4) {
                     medium.high(aconto1[0], amadness[0].cxz);
                     xtgraphics.stat(amadness[0], checkpoints, this.u[0], true, this.rd);
                  }
               }
...
//       try {
//           return new URL("/app/games/nfm-rar/");
