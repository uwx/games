...
public class GameSparker extends Panel implements Runnable {
   ...
   public int readcookie(String s) {
      ...
      try {
         BufferedReader br = new BufferedReader(new FileReader(new File("/files/nfm-mdx-v1/cookies/" + s)));
   ...
   public void loadbase(ContO[] aconto, Medium medium, Trackers trackers) {
      ...
      try {
         File file = new File("/app/games/nfm-mdx-v1/graphics/models.zipo");
   ...
   public void loadstage(
      ...
      try {
         File file = new File("/app/games/nfm-mdx-v1/stages/" + checkpoints.stage + ".txt");
         DataInputStream datainputstream = new DataInputStream(new FileInputStream(file));
         ...
         while ((s = datainputstream.readLine()) != null) {
            ...
            if (s1.startsWith("set")) {
               ...
               aconto[this.nob] = new ContO(
                  aconto1[k], this.getint("set", s1, 1), medium.ground - aconto1[k].grat, this.getint("set", s1, 2), this.getint("set", s1, 3)
               );
               if (s1.indexOf(")p") != -1) {
            ...
            if (s1.startsWith("chk")) {
               ...
               aconto[this.nob] = new ContO(
                  aconto1[l], this.getint("chk", s1, 1), medium.ground - aconto1[l].grat, this.getint("chk", s1, 2), this.getint("chk", s1, 3)
               );
               checkpoints.x[checkpoints.n] = this.getint("chk", s1, 1);
               checkpoints.z[checkpoints.n] = this.getint("chk", s1, 2);
               checkpoints.y[checkpoints.n] = medium.ground - aconto1[l].grat;
               if (this.getint("chk", s1, 3) == 0) {
                  checkpoints.typ[checkpoints.n] = 1;
               } else {
                  checkpoints.typ[checkpoints.n] = 2;
               }
   ...
   public void savecookie(String s, String s1) {
      try {
         PrintWriter pw = new PrintWriter(new File("/files/nfm-mdx-v1/cookies/" + s));
         pw.println(s1);
         pw.flush();
         pw.close();
      } catch (Exception var4) {
   ...
   public boolean cookieDir() {
      File f = new File("/files/nfm-mdx-v1/cookies");
      return f.exists() && f.isDirectory() ? true : f.mkdir();
   }

   public boolean initKeySettings() {
      try {
         File file = new File("/files/nfm-mdx-v1/KeySettings.txt");
