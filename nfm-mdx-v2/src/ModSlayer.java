...
public class ModSlayer {
   ...
   final void beattrack(ModTrackInfo modtrackinfo) {
      ...
      if ((modtrackinfo.effect & 1) != 0) {
         modtrackinfo.volume = modtrackinfo.volume + modtrackinfo.vol_slide;
      ...
      if ((modtrackinfo.effect & 2) != 0) {
         if ((modtrackinfo.period = modtrackinfo.period + modtrackinfo.port_down) > modtrackinfo.period_high_limit) {
      ...
      if ((modtrackinfo.effect & 4) != 0) {
         if ((modtrackinfo.period = modtrackinfo.period - modtrackinfo.port_up) < modtrackinfo.period_low_limit) {
            if (this.mod.s3m) {
               modtrackinfo.period = modtrackinfo.period_high_limit;
            } else {
      ...
      if ((modtrackinfo.effect & 32) != 0) {
         if (modtrackinfo.portto < modtrackinfo.period) {
            if ((modtrackinfo.period = modtrackinfo.period + modtrackinfo.port_inc) > modtrackinfo.portto) {
               modtrackinfo.period = modtrackinfo.portto;
            }
         } else if (modtrackinfo.portto > modtrackinfo.period && (modtrackinfo.period = modtrackinfo.period - modtrackinfo.port_inc) < modtrackinfo.portto) {
            modtrackinfo.period = modtrackinfo.portto;
         }
      ...
      if ((modtrackinfo.effect & 8) != 0) {
         modtrackinfo.vibpos = modtrackinfo.vibpos + (modtrackinfo.vib_rate << 2);
   ...
   final void mixtrack_16_mono(ModTrackInfo modtrackinfo, int[] ai, int i, int j) {
      ...
      int l = this.vol_adj[modtrackinfo.volume] * this.gain >> this.vol_shift + 8;
      int i1 = modtrackinfo.error;
      int j1 = modtrackinfo.pitch & 4095;
      int k1 = modtrackinfo.pitch >> 12;
      if (modtrackinfo.replen < 3) {
         int l1 = modtrackinfo.length;
         if (k >= l1) {
            return;
         }
         ...
         if (modtrackinfo.pitch < 4096) {
            while (k < l1 && i < j2) {
               ai[i++] += (abyte0[k] * (4096 - i1) + abyte0[k + 1] * i1) * l >> 12;
               int var14;
               k += k1 + ((var14 = i1 + j1) >> 12);
               i1 = var14 & 4095;
            }
         ...
         modtrackinfo.error = i1;
         modtrackinfo.position = k;
      } else {
         int i2 = modtrackinfo.replen + modtrackinfo.repeat;
         if (modtrackinfo.pitch < 4096) {
            while (j > 0) {
               if (k >= i2) {
   ...
   public byte[] turnbytes() throws IOException {
         ...
         if (this.oversample > 1) {
            ...
            if (this.oversample == 2) {
               ...
               for (int k = 0; k < i2; k++) {
                  ...
                  for (int i3 = 1; i3 < this.oversample; i3++) {
                     l2 += ai[j2++];
                  }

                  ai[k] = l2 / this.oversample;
               }
            }
         }
      ...
      for (int i1 = 2; i1 < l1; i1++) {
         ai2[i1] = (ai2[i1] + ai2[i1 - 2]) / 2;
      }

      for (int j1 = 57; j1 < l1; j1++) {
      ...
      for (int k2 = 0; k2 < l1; k2++) {
         abyte0[k2] = UlawUtils.linear2ulawclip(ai2[k2]);
      }
      ...
      if (l != 0 || i1 != 0) {
         switch (l) {
            case 0:
               ...
               while (modtrackinfo.period < period_set[j1]) {
                  if (++j1 >= 48) {
                     break;
                  }
               }
               ...
               modtrackinfo.arp[1] = period_set[j1 + (i1 & 15)];
               modtrackinfo.arp[2] = period_set[j1 + ((i1 & 240) >> 4)];
               modtrackinfo.arpindex = 0;
            ...
            case 4:
               if ((i1 & 15) != 0) {
                  modtrackinfo.vib_depth = i1 & 15;
               }

               if ((i1 & 240) != 0) {
                  modtrackinfo.vib_rate = (i1 & 240) >> 4;
               }
            ...
            case 10:
               modtrackinfo.vol_slide = ((i1 & 240) >> 4) - (i1 & 15);
            ...
            case 12:
               ...
               break;
            case 13:
               this.break_row = ((i1 & 240) >> 4) * 10 + (i1 & 15);
               this.row = 64;
               break;
            case 14:
               int k1 = i1 & 240;
