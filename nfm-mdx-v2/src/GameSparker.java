...
public class GameSparker extends Panel implements Runnable {
   ...
   public int readcookie(String s) {
      ...
      try {
         BufferedReader br = new BufferedReader(new FileReader(new File("/files/nfm-mdx-v2/cookies/" + s)));
   ...
   public void loadbase(ContO[] aconto, Medium medium, Trackers trackers) {
      ...
      try {
         File file = new File("/app/games/nfm-mdx-v2/graphics/models.zipo");
   ...
   public void loadstage(
      ...
      try {
         File file = new File("/app/games/nfm-mdx-v2/stages/" + checkpoints.stage + ".txt");
         DataInputStream datainputstream = new DataInputStream(new FileInputStream(file));
   ...
   public void savecookie(String s, String s1) {
      try {
         PrintWriter pw = new PrintWriter(new File("/files/nfm-mdx-v2/cookies/" + s));
         pw.println(s1);
         pw.flush();
         pw.close();
      } catch (Exception var4) {
   ...
   public boolean cookieDir() {
      File f = new File("/files/nfm-mdx-v2/cookies");
      return f.exists() && f.isDirectory() ? true : f.mkdir();
   }

   public boolean initKeySettings() {
      try {
         File file = new File("/files/nfm-mdx-v2/KeySettings.txt");
